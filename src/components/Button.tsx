...
const button = cva(
  'rounded-lg relative font-semibold shadow-lg transition-shadow duration-200 hover:shadow-xl p-[1px]',
  {
    variants: {
      intent: {
        default:
          'bg-zinc-900 shadow-zinc-200 hover:shadow-zinc-200 border-zinc-900',
        ...
        success:
          'bg-green-500 shadow-green-200 hover:shadow-green-200 border-green-500',
...
const buttonInner = cva('transition duration-100 px-6 py-3 rounded-[7px]', {
  variants: {
    intent: {
      default: '',
...
export const Button: React.FunctionComponent<ButtonProps> = ({
  ...
  return (
    <button
      className={button({ intent, small, outline })}
      {...props}
    >
      <div className={buttonInner({ intent, small, outline })}>{children}</div>
    </button>
  );
};
