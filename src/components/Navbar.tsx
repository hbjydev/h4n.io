...
const NavbarLink: React.FC<{
  ...
  href: string;
  toggleMenu: () => void;
}> = ({ label, href, toggleMenu }) => (
  <li>
    <Link
      className="text-zinc-500 transition duration-200 hover:text-zinc-800"
      href={href}
      onClick={toggleMenu}
    >
      {label}
...
const Navbar: React.FC<{ border?: boolean }> = ({ border }) => {
  ...
  let rootClass =
    'fixed flex h-20 w-full items-center md:hidden text-zinc-900 backdrop-blur-[5px] bg-white bg-opacity-[0.8] backdrop-saturate-[180%] md:flex transition-all duration-200 z-50';
  ...
  let rootClassDesktop =
    'fixed hidden h-16 min-w-full items-center justify-center text-zinc-900 backdrop-blur-[5px] bg-white bg-opacity-[0.8] backdrop-saturate-[180%] md:flex transition-all duration-200 z-50';
    ...
    <header>
      <div className={rootClass}>
        <div className="container flex items-center justify-between px-6">
          ...
          {menuOpen ? (
            <div className="backdrop-blue-lg scroll-y fixed left-0 top-[79px] h-fit min-h-[calc(100vh-79px)] w-full bg-white px-6 py-4 shadow-md backdrop-blur-xl">
              <ul className="flex flex-col gap-4 text-lg">
                <NavbarLink
                  label="Home"
                  href="/"
                  toggleMenu={setMenuOff}
                />
                <NavbarLink
                  label="Posts"
                  href="/posts"
